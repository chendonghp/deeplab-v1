...
class Resize(torch.nn.Module):
    ...
    def __init__(
        self, size, image_interpolation=Image.BILINEAR, mask_interpolation=Image.NEAREST
    ):
        super().__init__()
    ...
    def forward(self, image, mask):
        return F.resize(image, self.size, self.image_interpolation), F.resize(
            mask, self.size, self.mask_interpolation
        )
...
class RandomPerspective(torch.nn.Module):
    ...
    def __init__(
        self, distortion_scale=0.5, p=0.5, interpolation=Image.BILINEAR, fill=0
    ):
        super().__init__()
        self.p = p
    ...
    def forward(self, image, mask):
        if torch.rand(1) < self.p:
            ...
            return F.perspective(
                image, startpoints, endpoints, self.interpolation, self.fill
            ), F.perspective(
                mask, startpoints, endpoints, self.interpolation, self.fill
            )
        return image, mask
...
class RandomResizedCrop(torch.nn.Module):
    ...
    def forward(self, image, mask):
        ...
        return F.resized_crop(
            image, i, j, h, w, self.size, self.image_interpolation
        ), F.resized_crop(mask, i, j, h, w, self.size, self.mask_interpolation)
...
class RandomRotation(torch.nn.Module):
    ...
    def forward(self, image, mask):
        ...
        ), F.rotate(mask, angle, self.resample, self.expand, self.center, self.fill)
...
class RandomAffine(torch.nn.Module):
    ...
    def __init__(
        self, degrees, translate=None, scale=None, shear=None, resample=0, fillcolor=0
    ):
        super().__init__()
        self.degrees = _setup_angle(degrees, name="degrees", req_sizes=(2,))
